...
void TestScene::onAttachToEngine() {
    Scene::onAttachToEngine();

    // BZ::Sprite sprite;
    // sprite.position = { 0, 0 };
    // sprite.dimensions = { 10, 10 };
    // sprite.tintAndAlpha = { 1, 0, 0, 1 };
    //
    // entt::entity entity = ecsInstance.create();
    // ecsInstance.emplace<BZ::Sprite>(entity, sprite);
    ...
    entt::entity cameraEnt = ecsInstance.create();
    ecsInstance.emplace<BZ::Transform>(cameraEnt, glm::vec3(WINDOW_HALF_DIMS.x, WINDOW_HALF_DIMS.y, 0.0f));
    ecsInstance.emplace<BZ::OrthographicCamera>(cameraEnt, -WINDOW_HALF_DIMS.x, WINDOW_HALF_DIMS.x, -WINDOW_HALF_DIMS.y,
                                                WINDOW_HALF_DIMS.y);

    ecsInstance.emplace<BZ::CameraController2D>(cameraEnt, 1.0f, 400.0f, true, 45.0f);

    emitterEnt = ecsInstance.create();

    BZ::ParticleSystem2D particleSystem({ WINDOW_HALF_DIMS.x, WINDOW_HALF_DIMS.y });
    ...
    particleSystem.emitters.push_back({ { 0.0f, 0.0f }, 3'000, -1, ranges2, tex1 });
    particleSystem.emitters.push_back({ { 0.0f, 0.0f }, 5'000, -1, ranges, tex2 });

    particleSystem.start();

    ecsInstance.emplace<BZ::ParticleSystem2D>(emitterEnt, particleSystem);
}


void TestScene::onUpdate(const BZ::FrameTiming &frameTiming) {
    Scene::onUpdate(frameTiming);
...
//                                 "Sandbox/meshes/fireHydrant/Roughness.png", "Sandbox/meshes/fireHydrant/Height.png");
...
//    auto texRef = BZ::Texture2D::create(whiteTextureData, 1, 1, VK_FORMAT_R8G8B8A8_UNORM,
//    BZ::MipmapData::Options::DoNothing);
//    //auto texRef = BZ::Texture2D::create("Sandbox/textures/test.jpg", VK_FORMAT_R8G8B8A8_SRGB,
//    BZ::MipmapData::Options::Generate); BZ::Mesh sphereMesh("Sandbox/meshes/sphere.obj");
//
...
//
//    // scenes[activeScene].getEntities()[2].transform.yaw(frameTiming.deltaTime.asSeconds() * 10.0f,
//    BZ::Space::Local);
//
