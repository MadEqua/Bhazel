...
class Transform {
  public:
    ...
    void setRotationEuler(float x, float y, float z, Space space) {
        setOrientation(glm::quat(glm::radians(glm::vec3(x, y, z))), space);
    }
    void rotate(const glm::quat &quat, Space space);
    void rotate(float angle, const glm::vec3 &axis, Space space) {
