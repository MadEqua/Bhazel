...
void CameraControllerSystem::update(const FrameTiming &frameTiming, Scene &scene) {
    scene.getEcsInstance().view<Transform, OrthographicCamera, CameraController2D>().each(
        [&frameTiming](auto &transform, auto &camera, auto &controller) {
            if (controller.enableRotation) {
                if (Input::isKeyPressed(BZ_KEY_Q)) {
                    transform.roll(controller.cameraRotationSpeed * frameTiming.deltaTime.asSeconds(), Space::Parent);
                }
                if (Input::isKeyPressed(BZ_KEY_E)) {
                    transform.roll(-controller.cameraRotationSpeed * frameTiming.deltaTime.asSeconds(), Space::Parent);
                }
            ...
            if (positionChanged) {
                glm::vec3 movementScaled = movementDir * controller.cameraMoveSpeed * frameTiming.deltaTime.asSeconds();

                // TODO (Mistery alert): Space::Parent is behaving as Space::Local and vice-versa.
                // Only in this CameraController, other Controllers and Entities behave as expected, and they use the
                // Transform class equally.
                transform.translate(movementScaled, Space::Parent);
            }
        });
}
}