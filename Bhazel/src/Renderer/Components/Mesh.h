...
class Mesh {
  public:
    ...
    Mesh(Vertex vertices[], uint32 vertexCount, uint32 indices[], uint32 indexCount,
    ...
    const SubMesh &getSubMeshIdx(uint32 idx) const {
        BZ_ASSERT_CORE(idx < submeshes.size(), "Invalid index!");
        return submeshes[idx];
    }
    SubMesh &getSubMeshIdx(uint32 idx) {
        BZ_ASSERT_CORE(idx < submeshes.size(), "Invalid index!");
        return submeshes[idx];
    }
...
template <> struct std::hash<BZ::Mesh::Vertex> {
    size_t operator()(const BZ::Mesh::Vertex &vertex) const {
        return std::hash<glm::vec3>()(vertex.position) ^ std::hash<glm::vec3>()(vertex.normal) ^
               std::hash<glm::vec4>()(vertex.tangentAndDet) ^ vertex.texCoord[0] ^ vertex.texCoord[1];
    }
};