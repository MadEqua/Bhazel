...
TimeDuration::TimeDuration() : nanos(std::chrono::nanoseconds::zero()) {
}

TimeDuration::TimeDuration(uint64 nanos) : nanos(nanos) {
}

TimeDuration::TimeDuration(std::chrono::nanoseconds nanos) : nanos(nanos) {
}
...
/*-------------------------------------------------------------------------------------------*/
ScopedTimer::ScopedTimer(Fn &&callback) : callback(std::move(callback)) {
    timer.start();
}
