...
class Window {
  public:
    ...
    glm::ivec2 getDimensionsInt() const {
        return { static_cast<int>(data.dimensions.x), static_cast<int>(data.dimensions.y) };
    }
    glm::vec2 getDimensionsFloat() const {
        return { static_cast<float>(data.dimensions.x), static_cast<float>(data.dimensions.y) };
    }
