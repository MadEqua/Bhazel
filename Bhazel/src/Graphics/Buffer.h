...
enum class DataType { Float32, Float16, Int32, Int16, Int8, Uint32, Uint16, Uint8 };

enum class DataElements { Scalar, Vec2, Vec3, Vec4, Mat2, Mat3, Mat4 };
...
/*
 * Generic buffer of data. Will replicate data if considered dynamic (one replica per frame in-flight) to avoid race
 * conditions.
 */
class Buffer : public GpuObject<BufferHandles> {
  public:
    ...
    bool isMappable() const {
        return memoryType == MemoryType::CpuToGpu || memoryType == MemoryType::GpuToCpu ||
               memoryType == MemoryType::Staging;
    }
