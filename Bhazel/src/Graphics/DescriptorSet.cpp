...
DescriptorSetLayout::DescriptorSetLayout(const std::initializer_list<DescriptorDesc> &descs) : descriptorDescs(descs) {
...
void DescriptorSet::setConstantBuffers(const Ref<Buffer> buffers[], uint32 srcArrayCount, uint32 dstArrayOffset,
                                       uint32 binding, uint32 offsets[], uint32 sizes[]) {
    ...
    BZ_ASSERT_CORE(layout->getDescriptorDescs()[binding].arrayCount >= dstArrayOffset + srcArrayCount,
                   "Overflowing the array for binding {}!", binding);
    BZ_ASSERT_CORE(binding < layout->getDescriptorDescs().size(),
                   "Binding {} does not exist on the layout for this DescriptorSet!", binding);
    BZ_ASSERT_CORE(!buffers[0]->isReplicated() ||
                       (buffers[0]->isReplicated() &&
                        layout->getDescriptorDescs()[binding].type == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC),
                   "The buffer is effectively \"dynamic\" (there are internally created replicas because of memory "
                   "type), so the type on the layout needs to be VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC.");
...
void DescriptorSet::setCombinedTextureSampler(const Ref<TextureView> &textureView, const Ref<Sampler> &sampler,
                                              uint32 binding) {
    setCombinedTextureSamplers(&textureView, 1, 0, sampler, binding);
}

void DescriptorSet::setCombinedTextureSamplers(const Ref<TextureView> textureViews[], uint32 srcArrayCount,
                                               uint32 dstArrayOffset, const Ref<Sampler> &sampler, uint32 binding) {
    BZ_ASSERT_CORE(layout->getDescriptorDescs()[binding].type == VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
                   "Binding {} is not of type CombinedTextureSampler!", binding);
    BZ_ASSERT_CORE(layout->getDescriptorDescs()[binding].arrayCount >= dstArrayOffset + srcArrayCount,
                   "Overflowing the array for binding {}!", binding);
    BZ_ASSERT_CORE(binding < layout->getDescriptorDescs().size(),
                   "Binding {} does not exist on the layout for this DescriptorSet!", binding);
...
void DescriptorSet::setSampledTextures(const Ref<TextureView> textureViews[], uint32 srcArrayCount,
                                       uint32 dstArrayOffset, uint32 binding) {
    BZ_ASSERT_CORE(layout->getDescriptorDescs()[binding].type == VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE,
                   "Binding {} is not of type SampledTexture!", binding);
    BZ_ASSERT_CORE(binding < layout->getDescriptorDescs().size(),
                   "Binding {} does not exist on the layout for this DescriptorSet!", binding);
...
void DescriptorSet::setSamplers(const Ref<Sampler> samplers[], uint32 srcArrayCount, uint32 dstArrayOffset,
                                uint32 binding) {
    BZ_ASSERT_CORE(layout->getDescriptorDescs()[binding].type == VK_DESCRIPTOR_TYPE_SAMPLER,
                   "Binding {} is not of type Sampler!", binding);
    BZ_ASSERT_CORE(binding < layout->getDescriptorDescs().size(),
                   "Binding {} does not exist on the layout for this DescriptorSet!", binding);
