...
RenderPass::RenderPass(const std::initializer_list<AttachmentDescription> &descs,
    ...
    subPassDescs(subPassDescs), subPassDeps(subPassDeps) {
...
void RenderPass::init() {
    ...
    for (const SubPassDescription &subPassDesc : subPassDescs) {
        ...
        vkSubpassDesc.pDepthStencilAttachment =
            hasDepthStencilAttachment() ? (depthStencilVkAttachmentRefs.data() + preserveVkAttachmentIndicesOffset)
                                        : nullptr;
