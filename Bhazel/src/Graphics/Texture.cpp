...
static void generateMipmaps(CommandBuffer &comBuffer, const Texture &texture) {
    ...
    vkGetPhysicalDeviceFormatProperties(BZ_GRAPHICS_DEVICE.getPhysicalDevice().getHandle(), texture.getFormat(),
                                        &formatProperties);
    ...
    for (i = 1; i < texture.getMipLevels(); ++i) {
        ...
        comBuffer.pipelineBarrierTexture(texture, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
        ...
        comBuffer.blitTexture(texture, texture, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                              VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &blit, 1, VK_FILTER_LINEAR);

        // Layout transition previous mipmap from SRC_OPTIMAL to SHADER_READ_ONLY_OPTIMAL.
        comBuffer.pipelineBarrierTexture(texture, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,
...
static void copyBufferToImage(CommandBuffer &comBuffer, const Buffer &buffer, const Texture &texture,
                              uint32 bufferOffset, uint32 mipLevel, uint32 width, uint32 height) {
...
/*-------------------------------------------------------------------------------------------*/
TextureFormat::TextureFormat(VkFormat format) : format(format) {
}
...
Ref<Texture2D> Texture2D::create(const byte *data, uint32 width, uint32 height, TextureFormat format,
...
Ref<Texture2D> Texture2D::createRenderTarget(uint32 width, uint32 height, uint32 layers, uint32 mipLevels,
                                             TextureFormat format, VkImageUsageFlags additionalUsageFlags) {
    ...
    if (mipmapData.option == MipmapData::Options::Load) {
        ...
        for (uint32 mipIdx = 0; mipIdx < mipLevels; ++mipIdx) {
            ...
            const FileData fileData =
                loadFile(fullPath.c_str(), format.getChannelCount(), true, format.isFloatingPoint());
        ...
        commBuffer.pipelineBarrierTexture(*this, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0,
                                          VK_ACCESS_TRANSFER_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
                                          VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 0, mipLevels);
        ...
        for (uint32 mipIdx = 0; mipIdx < mipLevels; ++mipIdx) {
            ...
            copyBufferToImage(commBuffer, stagingBuffer, *this, stagingOffset, mipIdx, fileDatas[0].width,
                              fileDatas[0].height);
        ...
        commBuffer.pipelineBarrierTexture(*this, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,
                                          ...
                                          VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, 0, mipLevels);
        ...
        if (mipmapData.option == MipmapData::Options::Generate) {
            ...
            commBuffer.pipelineBarrierTexture(*this, VK_PIPELINE_STAGE_TRANSFER_BIT,
...
Texture2D::Texture2D(uint32 width, uint32 height, uint32 layers, uint32 mipLevels, TextureFormat format,
                     VkImageUsageFlags additionalUsageFlags) :
    Texture(format) {
...
Texture2D::Texture2D(VkImage vkImage, uint32 width, uint32 height, VkFormat vkFormat) : Texture(vkFormat) {
...
void Texture2D::createImage(bool hasData, MipmapData mipmapData, VkImageUsageFlags additionalUsageFlags) {
    ...
    if (hasData) {
        ...
        imageInfo.usage =
            format.isColor() ? VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT : VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT;
    }
    ...
    BZ_ASSERT_VK(vmaCreateImage(BZ_MEM_ALLOCATOR, &imageInfo, &allocInfo, &handle.imageHandle, &handle.allocationHandle,
                                nullptr));
}
    ...
    if (mipmapData.option == MipmapData::Options::Load) {
        ...
        for (uint32 mipIdx = 0; mipIdx < mipLevels; ++mipIdx) {
            ...
            for (uint32 faceIdx = 0; faceIdx < 6; ++faceIdx) {
                ...
                const FileData fileData =
                    loadFile(fullPath.c_str(), format.getChannelCount(), false, format.isFloatingPoint());
        ...
        for (uint32 mipIdx = 0; mipIdx < mipLevels; ++mipIdx) {
            ...
            copyBufferToImage(commBuffer, stagingBuffer, *this, copyOffset, mipIdx, fileDatas[mipIdx * 6].width,
                              fileDatas[mipIdx * 6].height);
...
Ref<TextureView> TextureView::create(const Ref<Texture2D> &texture2D, uint32 baseLayer, int layerCount, uint32 baseMip,
                                     int mipCount) {
...
TextureView::TextureView(const Ref<Texture2D> &texture2D) : texture(texture2D), format(texture2D->getFormat()) {
...
TextureView::TextureView(const Ref<Texture2D> &texture2D, TextureFormat format) : texture(texture2D), format(format) {
...
TextureView::TextureView(const Ref<Texture2D> &texture2D, uint32 baseLayer, int layerCount, uint32 baseMip,
                         int mipCount) :
    texture(texture2D),
    ...
    BZ_ASSERT_CORE(layerCount < 0 || baseLayer + layerCount <= texture2D->getLayers(),
                   "Invalid baseLayer + layerCount!");
...
TextureView::TextureView(const Ref<TextureCube> &textureCube) : texture(textureCube), format(textureCube->getFormat()) {
