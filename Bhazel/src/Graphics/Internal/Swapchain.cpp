...
void Swapchain::aquireImage(const Ref<Semaphore> &imageAvailableSemaphore) {
    ...
    VkResult result = vkAcquireNextImageKHR(device->getHandle(), handle, 0, imageAvailableSemaphore->getHandle(),
...
void Swapchain::internalInit() {
    ...
    BZ_LOG_CORE_INFO("Swapchain needs a minImageCount of {} and a maxImageCount of {}. Picked an image count of {}.",
                     swapchainSupport.surfaceCapabilities.minImageCount,
    ...
    if (queueContainer.graphics().getFamily().getIndex() != queueContainer.present().getFamily().getIndex()) {
        uint32_t queueFamilyIndicesArr[] = { queueContainer.graphics().getFamily().getIndex(),
                                             queueContainer.present().getFamily().getIndex() };
...
void Swapchain::createFramebuffers() {
    ...
    for (size_t i = 0; i < swapChainImages.size(); i++) {
        ...
        framebuffers[i] =
            Framebuffer::create(renderPass, { textureViewRef }, glm::uvec3(extent.width, extent.height, 1));

        // Force view to linear format.
        // auto textureViewLinearRef = TextureView::create(textureRef, imageFormatLinear);
        // framebuffersLinear[i] = Framebuffer::create(renderPassLinear, { textureViewLinearRef },
        // glm::uvec3(extent.width, extent.height, 1));
    }
}
