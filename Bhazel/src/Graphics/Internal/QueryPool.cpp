...
void QueryPool::init(const Device &device, VkQueryType queryType, uint32 queryCount,
...
void QueryPool::getResults(uint32 firstQuery, uint32 queryCount, uint32 dataSize, void *data, uint32 stride,
                           VkQueryResultFlags flags) {
    VkResult res =
        vkGetQueryPoolResults(device->getHandle(), handle, firstQuery, queryCount, dataSize, data, stride, flags);
    if (res == VK_NOT_READY) {
