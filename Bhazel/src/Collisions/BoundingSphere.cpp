...
BoundingSphere::BoundingSphere(const BoundingSphere &other) :
    IBoundingVolume(BoundingVolumeType::Sphere), center(other.center), radius(other.radius) {
}

BoundingSphere::BoundingSphere(const glm::vec3 *points, uint32 count) : IBoundingVolume(BoundingVolumeType::Sphere) {
    empty();
...
BoundingSphere::BoundingSphere(const BoundingSphere &other, const glm::mat4 &transform) : BoundingSphere(other) {
    this->transform(transform);
}

BoundingSphere::BoundingSphere(const glm::vec3 &center, float radius) : IBoundingVolume(BoundingVolumeType::AABB) {
    this->center = center;
    this->radius = radius;
}
