    ...
    IntersectionResult IBoundingVolume::intersects(const IBoundingVolume &other) const {
        if (type == BoundingVolumeType::AABB) {
            if (other.getType() == BoundingVolumeType::AABB) {
                ...
                return CollisionUtils::intersects(static_cast<const AABB&>(*this), static_cast<const BoundingSphere&>(other));
            }
        }
        else if (type == BoundingVolumeType::Sphere) {
            if (other.getType() == BoundingVolumeType::AABB) {
                return CollisionUtils::intersects(static_cast<const BoundingSphere&>(*this), static_cast<const AABB&>(other));
            }
            else if (other.getType() == BoundingVolumeType::Sphere) {
                return CollisionUtils::intersects(static_cast<const BoundingSphere&>(*this), static_cast<const BoundingSphere&>(other));
            }
        }
